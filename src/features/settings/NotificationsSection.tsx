        ...
        <div
          ...
          className={`flex items-center justify-between px-4 py-3.5 border transition-all duration-200 ${
            n.on
              ...
              : 'border-silver dark:border-[#3D3268] hover:border-blush/40 hover:bg-silver/10 dark:hover:bg-silver/8'
          }`}
        >
          <div className="flex items-center gap-3">
            <div className={`w-1.5 h-8 shrink-0 transition-colors ${n.on ? 'bg-gold' : 'bg-silver'}`} />
            <div>
              <p className={`text-sm font-semibold transition-colors ${n.on ? 'text-dark dark:text-white' : 'text-zinc-500 dark:text-silver/60'}`}>
                {n.label}
              </p>
