    ...
    <Modal onClose={onClose} aria-label="Vendor details" className="flex flex-col max-h-[90svh]">
      ...
      <div className="overflow-y-auto flex-1 min-h-0 px-6 pt-5 pb-6 space-y-5">
        ...
        <div className="flex items-stretch gap-0 border border-gold/20 overflow-hidden">
          ...
          <div className="flex-1 px-4 py-3.5 bg-gold/5">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="text-sm font-bold text-white">{vendor.name}</p>
                <p className="text-[10px] text-silver/40 uppercase tracking-widest mt-0.5">{vendor.category}</p>
              </div>
            ...
            <div className="flex items-center gap-0.5 mt-2">
              {Array.from({ length: 5 }).map((_, i) => (
                <StarIcon key={i} size={11} filled={i < vendor.rating}
                  className={i < vendor.rating ? 'text-gold' : 'text-silver/20'} />
              ))}
              <span className="ml-1.5 text-[10px] font-semibold text-silver/40">{vendor.rating}.0</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          {vendor.notes && (
            <div className="px-4 py-3 bg-silver/5 border border-silver/10">
              <p className="text-[10px] font-bold text-silver/40 uppercase tracking-widest mb-1.5">Notes</p>
              <p className="text-xs text-silver/60 leading-relaxed">{vendor.notes}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex-shrink-0 flex gap-3 px-6 py-4 border-t border-gold/10">
        ...
        {vendor.status !== 'booked' ? (
          ...
          <div className="flex-1 flex items-center justify-center gap-2 px-4 py-2 text-xs font-bold text-gold bg-gold/8 border border-gold/20">
            ✓ Confirmed Booking
          </div>
        )}
      </div>
...
const DetailRow = ({ icon, label, value }: { icon: string; label: string; value: string }) => (
  <div className="flex items-center gap-3 px-4 py-2.5 border border-silver/10">
    <span className="text-blush text-sm w-5 shrink-0">{icon}</span>
    <div className="flex-1 min-w-0">
      <p className="text-[10px] font-bold text-silver/35 uppercase tracking-widest">{label}</p>
      <p className="text-xs text-silver/70 mt-0.5 truncate">{value}</p>
    </div>
  </div>
);
