...
export const GuestChip = ({ guest, overlay = false }: Props) => {
  ...
  return (
    <div
      ...
      className={[
        'flex items-center gap-2 px-2.5 py-1.5 bg-white dark:bg-[#0F0C24] border rounded-lg select-none transition-all',
        overlay
          ? 'border-gold/60 shadow-2xl shadow-black/30 cursor-grabbing rotate-1 scale-105'
          : isDragging
          ? 'border-gold/30 opacity-30 cursor-grabbing'
          : 'border-silver/25 dark:border-silver/15 hover:border-blush dark:hover:border-gold/40 cursor-grab active:cursor-grabbing shadow-sm',
      ].join(' ')}
    >
      <span className="w-5 h-5 rounded-full bg-gold/20 flex items-center justify-center text-[9px] font-black text-gold shrink-0">
        {initials}
      </span>
      <span className="text-[11px] font-semibold text-dark dark:text-silver/80 truncate max-w-[100px]">{guest.name}</span>
