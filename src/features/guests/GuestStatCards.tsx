...
export const GuestStatCards = ({ total, confirmed, pending, declined }: GuestStatCardsProps) => (
  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 stagger-children">
    {[
      ...
      <div key={label} className={`stat-card bg-card border border-silver dark:border-[#2a2f33] border-b-2 ${border} px-5 py-5 lift-deep shadow-crystal`}>
        <p className={`text-3xl font-black num-pop ${val}`}>{value}</p>
        <p className="text-xs font-bold text-dark dark:text-background mt-2">{label}</p>
