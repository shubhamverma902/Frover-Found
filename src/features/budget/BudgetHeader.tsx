...
export const BudgetHeader = ({ totalPct, onAddExpense }: BudgetHeaderProps) => (
  <div className="bg-dark p-[3px] glow-gold-strong relative">
    ...
    <div className="border border-gold/20 px-6 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative overflow-hidden">
      ...
      <div className="relative">
        ...
        <p className="text-xs text-silver/50 mt-1">{totalPct}% of total budget utilised</p>
      </div>
      <button
        onClick={onAddExpense}
        className="relative self-start sm:self-auto px-5 py-2.5 text-xs font-bold bg-gold text-dark hover:bg-gold/85 transition-all hover:shadow-[0_4px_16px_rgba(228,188,98,0.45)]"
      >
        + Add Expense
      </button>
    </div>
