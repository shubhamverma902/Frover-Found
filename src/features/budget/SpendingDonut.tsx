...
const CustomTooltip = ({ active, payload }: { active?: boolean; payload?: { payload: TooltipPayload }[] }) => {
  ...
  return (
    <div className="bg-[#1c2226] border border-[#E4BC62]/20 px-3 py-2 shadow-xl">
      <div className="flex items-center gap-2 mb-1">
        <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: color }} />
        <span className="text-[11px] font-bold text-[#DDDED9]/80">{name}</span>
      </div>
...
export const SpendingDonut = ({ categories, spent, total }: Props) => {
  ...
  return (
    <div className="bg-[#23292E] border border-[#E4BC62]/12 p-6">

      {/* Section header */}
      <div className="flex items-center gap-2 mb-6">
        <span className="text-[#E4BC62]/50 text-[9px]">◈</span>
        <p className="text-[10px] font-bold text-[#DDDED9]/50 uppercase tracking-[0.35em]">Spend by Category</p>
      </div>

      {isEmpty ? (
        <div className="flex flex-col items-center justify-center py-10 gap-2">
          <span className="text-4xl text-[#DDDED9]/10">◎</span>
          <p className="text-xs text-[#DDDED9]/30">No expenses recorded yet</p>
        </div>
      ) : (
        <div className="flex flex-col lg:flex-row items-center gap-8">
          ...
          <div className="relative shrink-0 w-52 h-52">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <p className="text-[9px] font-bold text-[#DDDED9]/35 uppercase tracking-[0.3em]">Spent</p>
              ...
              <div key={entry.name} className="flex items-center gap-2.5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline justify-between gap-2">
                    <p className="text-[11px] font-semibold text-[#DDDED9]/70 truncate">{entry.name}</p>
                    <p className="text-[11px] font-black text-white shrink-0">₹{fmt(entry.value)}</p>
                  </div>
                  {/* Mini bar */}
                  <div className="mt-0.5 h-[3px] bg-[#DDDED9]/8 rounded-full overflow-hidden">
                    <div
                ...
                <span className="text-[10px] text-[#DDDED9]/30 shrink-0 w-7 text-right">{entry.pct}%</span>
              </div>
            ))}
          </div>
