...
interface LogoProps {
  size?: 'sm' | 'md' | 'lg';
  /** light = white text (always-dark bg) | dark = dark text (always-light bg) | auto = adapts to mode */
  theme?: 'dark' | 'light' | 'auto';
  href?: string;
}

export function Logo({ size = 'md', theme = 'auto', href = '/' }: LogoProps) {
  ...
  return (
    <Link href={href} className="inline-flex items-center gap-2.5 group select-none">
      ...
      >
        {/* Outer petals — adapts: charcoal in light, light plum in dark */}
