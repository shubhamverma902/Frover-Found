...
export const HeroSection = ({ primaryHref, primaryLabel }: HeroSectionProps) => (
  <section className="relative h-screen min-h-[600px]">
    ...
    {/* Gradient overlay */}
    <div className="absolute inset-0 bg-gradient-to-b from-dark/80 via-dark/40 to-dark/75 z-10 pointer-events-none" />
    ...
    <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center text-white px-6 pt-16 pointer-events-none">
      <span className="animate-fade-in inline-flex items-center gap-2 mb-5 px-5 py-2 rounded-full bg-white/10 backdrop-blur-sm border border-gold/40 text-gold text-sm font-medium">
      ...
      <div className="animate-fade-in-up-d3 mt-10 flex flex-col sm:flex-row gap-4 items-center pointer-events-auto">
        <Link
          href={primaryHref}
          className="px-8 py-4 rounded-full bg-gold hover:bg-gold/90 text-dark font-semibold text-sm transition-all duration-200 hover:scale-105 shadow-lg shadow-dark/30"
        >
          {primaryLabel}
        </Link>
        <a
          href="#gallery"
          className="px-8 py-4 rounded-full border border-gold/50 hover:bg-gold/10 text-gold font-semibold text-sm transition-all duration-200"
        >
          See Our Work
        </a>
      </div>
    ...
    <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 pointer-events-none animate-float">
      <div className="w-6 h-10 rounded-full border-2 border-gold/50 flex items-start justify-center pt-2">
        <div className="w-1 h-2.5 rounded-full bg-gold/70" />
      </div>
    </div>
  </section>
);
