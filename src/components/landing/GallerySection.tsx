...
export const GallerySection = () => (
  <section id="gallery" className="py-24 px-6 bg-silver dark:bg-[#1E1840]">
    <div className="max-w-6xl mx-auto">
      <div className="text-center mb-4">
        <span className="inline-block mb-3 text-sm font-semibold text-gold uppercase tracking-widest">
          Our Work
        </span>
        <h2 className="text-4xl font-bold text-dark dark:text-white">Magical Moments</h2>
        <p className="mt-4 text-zinc-500 dark:text-silver/65 max-w-xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-14 pt-10">
        {GALLERY_IMAGES.map((img, i) => (
          <div
            key={i}
            className={`group relative bg-white dark:bg-[#2A1F52] p-2.5 pb-9 rounded-[2px] shadow-2xl shadow-dark/30 cursor-pointer
          ...
          >
            ...
            <p className="text-center mt-2 text-[11px] font-medium text-zinc-400 dark:text-silver/65 tracking-widest uppercase">
              {img.label}
            </p>
          </div>
        ))}
